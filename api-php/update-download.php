...
$payload = [];
if (isset($body['title'])) $payload['title'] = $body['title'];
if (isset($body['description'])) $payload['description'] = $body['description'];
if (!empty($body['file']) && str_starts_with($body['file'], 'http')) {
    $payload['file'] = $body['file'];
}

$url = "https://{$domain}.microcms.io/api/v1/downloads/{$id}" . ($is_draft ? '?status=draft' : '');
